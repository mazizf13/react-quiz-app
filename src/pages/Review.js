...
const Review = ({ answer, index, setDisplay, handlePrev, handleNext }) => {
  return (
    <div className="h-full relative p-4 md:p-[54px] font-urbanist flex flex-col gap-[20px]">
      <div className="flex text-primary text-[5vw] sm:text-[4vw] md:text-[32px] justify-center text-center">
        {`Question ${index + 1} : ${questions[index].question}`}
      </div>
      <div className="w-full flex justify-center">
        <div className="h-auto md:h-[161px] w-[80vw] sm:w-[60vw] md:w-[500px] rounded-[20px] flex text-primary p-5 cursor-pointer bg-dark border-solid border-4 border-white">
          {answer[index + 1]
            ? `${answer[index + 1]} . ${questions[index].options[answer[index + 1]]}`
            : 'You didn"t answer this question yet'}
        </div>
      </div>
      <div className="flex items-center justify-center gap-2 text-primary text-[8vw] sm:text-[6vw] md:text-[64px]">
      ...
      <div className="text-center text-primary text-[4vw] sm:text-[3vw] md:text-[20px]">
        The Answer : {questions[index].options[questions[index].answer]}
      </div>
      <div className="absolute bottom-[20px] sm:bottom-[60px] right-[20px] sm:right-[54px] flex justify-end mx-[20px] mt-5 gap-5 cursor-pointer">
        {index !== 0 && (
          <svg
            onClick={handlePrev}
            xmlns="http://www.w3.org/2000/svg"
            ...
            height="60"
            viewBox="0 0 72 73"
            className="bg-dark rounded-full border-solid border-[6px] fill-primary border-primary hover:border-white hover:scale-110 hover:shadow-2xl hover:fill-white "
            fill="#EAB2A0"
          >
        ...
        <div
          ...
          className="text-[6vw] sm:text-[5vw] md:text-[36px] bg-dark flex items-center px-4 text-primary border-[6px] border-primary hover:scale-110 hover:text-white hover:border-white"
        >
          View Score
        </div>
      </div>
